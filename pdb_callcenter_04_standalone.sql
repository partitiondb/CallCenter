...
where dbid in (db_id('CallGate'),db_id('AnyTradeDB'),db_id('BestTradeDB'),db_id('GoTradeDB'),db_id('CallGlobalDB'),db_id('CallCommonDB'),db_id('CallLogDB'),db_id('RepCallGate'),db_id('RepAnyTradeDB'),db_id('RepBestTradeDB'),db_id('RepGoTradeDB'),db_id('RepCallGlobalDB'),db_id('RepCallCommonDB'),db_id('RepCallLogDB')) and cmd = 'AWAITING COMMAND' and spid <> @@spid;
exec(@sql);
end;
go
...
if db_id('CallLogDB') 		is not null drop database CallLogDB;
...
if db_id('RepCallLogDB') 	is not null drop database RepCallLogDB;
...
exec PdbcreatePartition 'CallGate','CallLogDB',@DatabaseTypeId=4,@StandAloneSchemaName='Log';
...
create table Log.CallLog
	...
	,	CustomerId			bigint
	,	Company				nvarchar(128)
	,	CallCenterNumber	nvarchar(16)
	,	CustomerFirstName	nvarchar(128)
	,   CustomerLastName	nvarchar(128)
...
create index CallLog_01_IX on Log.CallLog (Time);
create index CallLog_02_IX on Log.CallLog (CustomerId);
create index CallLog_03_IX on Log.CallLog (Company);
create index CallLog_04_IX on Log.CallLog (CallCenterNumber);
create index CallLog_05_IX on Log.CallLog (CustomerEMail);
...
exec PdbcreatePartition 'RepCallGate','RepCallLogDB',@DatabaseTypeId=4,@StandAloneSchemaName='Log';
...
exec PdbsyncSourceTable 'RepCallGate','Log','CallLog','CallGate';
